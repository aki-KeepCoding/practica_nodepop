...
Helpers.stdRes = {
  ...
  responseWithImg: function (responseObj, req) {
    console.log('1', responseObj)
    var hostname = req.headers.host
    if (responseObj.foto) {
      responseObj.foto = hostname + config.imgsBaseURL + responseObj.foto
    } else if (Array.isArray(responseObj)) {
      responseObj = responseObj.map(function (obj) {
        if (obj.foto) {
          obj.foto = obj.foto = hostname + config.imgsBaseURL + obj.foto
        }
        return obj
      })
    }
